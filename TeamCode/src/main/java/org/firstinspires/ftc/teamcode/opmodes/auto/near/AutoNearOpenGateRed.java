package org.firstinspires.ftc.teamcode.opmodes.auto.near;
...
import org.firstinspires.ftc.teamcode.opmodes.auto.path.AutoPathNearOpenGate;
import org.firstinspires.ftc.teamcode.util.PedroDrawing;

@Config
@Autonomous(name = "Auto (Near Open Gate)(Red)", group = "Competition", preselectTeleOp = "TeleOpTuner")
public class AutoNearOpenGateRed extends CommandOpMode {
    ...
    // ==================== Robot & Paths ====================
    private Robot robot;
    private AutoPathNearOpenGate paths; // 使用新的路徑類別
    ...
    public void initialize() {
        ...
        // 3. 初始化路徑
        paths = new AutoPathNearOpenGate(robot.follower);
    ...
    private SequentialCommandGroup buildAutoSequence() {
        return new SequentialCommandGroup(
                ...
                new ParallelDeadlineGroup(
                        new FollowPathCommand(robot.follower, paths.spikeIII),
                        new IntakeCommand(robot.intake, ()-> true)
                ),

                // 2.2 Open Gate 動作 (Sample 3 後的短路徑)
                new FollowPathCommand(robot.follower, paths.openGate),

                // 2.3 移動到射擊點並瞄準
                debugWait("Press A to Shoot Spike 3"),
                new DriveAndAim(robot, paths.shootAfterSparkIII), // 名稱變更：shootAfterSpark3 -> shootAfterSparkIII
                shootAndClean(),
                ...
                // 3.1 接近 (Spark II)
                new FollowPathCommand(robot.follower, paths.spikeII),

                // 3.2 進入並吸取 (Intake Spark II)
                new ParallelDeadlineGroup(
                        new FollowPathCommand(robot.follower, paths.intakeSpikeII),
                        new IntakeCommand(robot.intake, ()-> true)
                ),

                // 3.3 Open Gate 動作
                new FollowPathCommand(robot.follower, paths.openGate2).setGlobalMaxPower(0.5), // 稍微降速確保穩定

                // 3.4 移動到射擊點並瞄準
                new DriveAndAim(robot, paths.shootAfterSpikeII),
                shootAndClean(),
                ...
                // 4.1 接近 (Spark I)
                new FollowPathCommand(robot.follower, paths.spikeI).setGlobalMaxPower(1),

                // 4.2 進入並吸取 (Intake Spark I)
                new ParallelDeadlineGroup(
                ...
                // 4.3 Open Gate 動作
                new FollowPathCommand(robot.follower, paths.openGate1,0.007),

                // 4.4 移動到射擊點並瞄準
                new DriveAndAim(robot, paths.shootAfterSpikeI),
                shootAndClean(),


                // --- Step 5: Park (停車) ---
                logState("🅿️ Parking..."),
                new FollowPathCommand(robot.follower, paths.park), // 名稱變更：park -> park
